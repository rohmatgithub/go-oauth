...
var (
	...
	ValidOperatorUser    map[string]DefaultOperator
)
...
func GenerateValidOperator() {
	...
	ValidOperatorUser = map[string]DefaultOperator{
		"username":   {DataType: "char", Operator: []string{"eq", "like"}},
		"first_name": {DataType: "char", Operator: []string{"eq", "like"}},
		"last_name":  {DataType: "char", Operator: []string{"eq", "like"}},
	}
}
...
func (input *GetListRequest) ValidateInputPageLimitAndOrderBy(validLimit []int, validOrderBy []string) model.ErrorModel {
	...
	if input.OrderBy == "" {
		...
		if !util.ValidateStringContainInStringArray(validOrderBy, orderBySplit[0]) {
			return model.GenerateFormatFieldError(constanta.OrderBy)
		}
...
func isOperatorValid(key string, value string, operator string, validOperator map[string]DefaultOperator) bool {
	if validOperator[key].Operator == nil {
		...
		return util.ValidateStringContainInStringArray(validOperator[key].Operator, operator)
	}
}
